...
// Same as above
// just as per leetcode the elements of candidates are unique
// so no need of removing the duplicate elements
// TC : O(2^n*k), k = average length of tans, for insertion into ans it takes time k
class Solution1 {
private:
    void recursion(vector<int>& candidates, int target, int ind, vector<vector<int>> &ans,
                    vector<int> &tans){
        int n = candidates.size();
        if(ind==n){
            if(target==0) ans.push_back(tans);
            return;
        }
        if(candidates[ind]<=target){
            tans.push_back(candidates[ind]);
            recursion(candidates,target-candidates[ind],ind,ans,tans);
            tans.pop_back();
        }
        recursion(candidates,target,ind+1,ans,tans);
        return;
    }
public:
    vector<vector<int>> combinationSum(vector<int>& candidates, int target) {
        vector<vector<int>> ans;
        vector<int> tans;
        recursion(candidates,target,0,ans,tans);
        return ans;
    }
};
