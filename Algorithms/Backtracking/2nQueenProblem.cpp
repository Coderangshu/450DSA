...
// 32 Bits used
// GOOD EXPLANATION BUT WRONG SOLUTION FOR n>4
...
// Need to keep track of the rowmask and all the upper and lower diagonals not just the
// previous column's as wrongly done above
// The number of columns is n, the number of 45° diagonals is 2 * n - 1,
// the number of 135° diagonals is also 2 * n - 1. When reach [row, col], the column No. is col,
// the 45° diagonal No. is row + col and the 135° diagonal No. is n - 1 + col - row.
// We can use three bitmasks to indicate if the column or the diagonal had a queen before,
// if not, we can put a queen in this position and continue.
class Solution {
private:
    vector<vector<string>> ans;
    
	// rowMask keeps track of all the previous rows filled with queens
	// ddiagMask keeps track of all the previous down diagonal rows filled with queens
	// udiagMask keeps track of all the previous upper diagonal rows filled with queens
    void recursion(vector<string> &board, int col, int rowMask, int ddiagMask, int udiagMask){
        int n = board.size();
        int allFilled = (1<<n)-1;
		
		// if all filled in rowMask means all queens placed
		// thus put into answer and continue
        if(allFilled==rowMask){
            ans.push_back(board);
            return;
        }
		
		// get the safe rows, i.e. the rows wherein queens can yet be placed
        int safeRows = allFilled&(~rowMask);
        while(safeRows){
			// get the first safe row number to place the queen
            int curRowLog = safeRows&(-safeRows);
            int curRow = (int)log2(curRowLog);
			
			// this row already used thus remove from safe rows
            safeRows &= safeRows-1;
			
			// now for the particular "safe" row check whether the corresponding upper 
			// and lower diagonals are empty or not if empty then can place queen
			// else continue
            if(!(ddiagMask&(1<<(curRow+col))) and !(udiagMask&(1<<(n-1+col-curRow)))){
                board[curRow][col] = 'Q';
                recursion(board,col+1,rowMask|curRowLog,
                          ddiagMask|(1<<(curRow+col)),udiagMask|(1<<(n-1+col-curRow)));
                board[curRow][col] = '.';
            }
        }
        return;
