...
// Method used is complete same as 8BSTFromPreorder.cpp
// the advanced method using the range whose time complexity
// is O(N)
...
Node* constructTreeUtil(int pre[], int &preIndex, int key,int min, int max, int size){
    ...
    if (key > min && key < max) {
        ...
        if (preIndex < size) root->left = constructTreeUtil(pre, preIndex,pre[preIndex],min, key, size);
        if (preIndex < size) root->right = constructTreeUtil(pre, preIndex,pre[preIndex],key, max, size);
    }
    return root;
}
