...
TreeNode* create(vector<int> &pre, int &ind, int mn, int mx, int size) {
	if(ind>=size) return nullptr;
	if(mn<pre[ind] and pre[ind]<mx) {
		TreeNode *root = new TreeNode(pre[ind++]);
		root->left = create(pre,ind,mn,root->val,size);
		root->right = create(pre,ind,root->val,mx,size);
		return root;
	}
	else return nullptr;
}
