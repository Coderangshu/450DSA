...
class Solution
{
    ...
    void topViewMap(Node *root, int horDis, int level, map<int, pair<int, int>> &mp){
        ...
        // or if node already present with the current horDis then we check
        ...
        if(mp.count(horDis)==0 or mp[horDis].second>level) mp[horDis] = make_pair(root->data,level);
